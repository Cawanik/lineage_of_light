# ==========================================
# build_menu.gd — Меню строительства, сука
# ==========================================
# _ready() — прячет меню и подгружает конфиг зданий, инициализация кнопок
# _build_buttons() — генерит кнопки из конфига, пропуская несстроительную хуйню
# _on_item_pressed(building_type) — вызывается при нажатии, шлёт сигнал наверх
# toggle_menu() — открыть/закрыть это ёбаное меню
# _input(event) — клавиша B тоглит меню, чё тут непонятного блять
# ==========================================
